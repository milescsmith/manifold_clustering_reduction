...
def read(*names, **kwargs):
    with io.open(
        join(Path(__file__).parent, *names),
...
setup(
    name='mass-cytometry-umap',
    use_scm_version={
        'local_scheme': 'dirty-tag',
        'write_to': 'src/mass_cytometry_reduction/_version.py',
    ...
    package_dir={'mcr': 'src/mass_cytometry_reduction'},
    py_modules=[Path(path).stem for path in glob('src/mass_cytometry_reduction/*.py')],
    ...
    entry_points={
        'console_scripts': [
            'mcr = mass_cytometry_reduction.cli:main',
        ]
    },
)
